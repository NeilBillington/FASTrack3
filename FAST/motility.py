...
#Tural Aksel
#05/30/13
# NB modified to remove cv2 requirement 
# edited Oct 2020 Neil B to work on python 3.8
# Feb23 Edited to work on M2 python 3.10 
...
def stack_to_tiffs(fname,frame_rate=1.0):
    ...
    #Read all the frames
    # changed by neil b to read tiffs using tifffile instead
    tiff_frames  = tiff.imread(fname)
    ...
    for i in range(num_frames):
        ...
        max=np.amax(tiff_frames[i])
        min=np.amin(tiff_frames[i])
        imagetemp=255*((tiff_frames[i]-min)/(max-min))
     # line below should fix discrepancy with stack2 and nikon2 convs? - NB   
        imagetemp=np.round(imagetemp,0)
        imagetemp=imagetemp.astype('uint8')
        imwrite(fout,imagetemp)
        #Write elapsed times
...
class Motility:
    def __init__(self):
        ...
        self.dx               = 130.00 #Pixel length in nm (from sellers tirf)
        ...
        self.minvfil          = 0     #Minimum average path velocity of the filament in nm/s (Default:80 nm/s)        
        self.min_velocity     = 0     #Minimum average path velocity of the filament in nm/s (Default:80 nm/s)
        ...
        self.filt_len_vel     = [] 	  # keeps filtered velocities -neil B
        self.mean_len_vel     = [] 	  # keeps data of filaments -neil B
        self.final_len_vel    = []    # keeps filament data after filtering -neil B        
    ...
    def read_metadata(self):
        ...
        '''
        fname = self.directory+os.sep+'metadata.txt'
    ...
    def read_frame_links(self):
        ...
        '''
        if not self.force_analysis and os.path.exists(self.directory+os.sep+'links.npy'):
            try:
                self.frame_links      = np.load(self.directory+os.sep+'links.npy', allow_pickle=True)
    ...
    def reconstruct_skeleton_images(self):
        ...
        if not os.path.isfile(self.directory+os.sep+'paths_2D.png'):
            return
        ...
        for i in range(len(self.frame_links)):
            ...
            for link in self.frame_links[i]:
                ...
                py.arrow(mp_1[1],mp_1[0],arrow_length*mp_diff[1],arrow_length*mp_diff[0],color='r',head_width=1,head_length=2,alpha=1.0)
                py.text(mp_1[1],mp_1[0],"%.f"%(velocity),fontsize=6,color='k',alpha=1.0)
            ...
            skeleton_fname = self.directory+os.sep+'skeletons_%03d.png'%(i)
            paths_fname    = self.directory+os.sep+'paths_2D.png'
    ...
    def path_velocities(self,num_points, percent_tolerance, min_path_length, max_length, fil_tolerance, minvfil):
        '''
        Calculate average velocities over a path with num_points points
        '''
        self.full_len_vel = []
        self.max_len_vel  = []
        self.filt_len_vel = []   # added by NeilB
        self.mean_len_vel = []   # added by NeilB
        self.final_len_vel = []  # added by NeilB
        filidcounter = 0         # added by NeilB
      ...
      # NeilB next calculate the average filament length using all
      # do same for vels and get std of each        
            
            array_actual = []
            
            actual_avelen = np.mean(array_len)
            actual_stdlen = np.std(array_len)
            actual_avevel = np.mean(array_vel)
            actual_stdvel = np.std(array_vel)
            actual_maxvel = np.max(array_vel)
            ...
            # next part added NeilB
            maxvsmooth   = np.amax(array_smooth[:,1])
            smavvel      = np.mean(array_smooth[:,1])
            smsdvel      = np.std(array_smooth[:,1])
            untolval     = (actual_stdvel/actual_avevel)*100
            smtolval     = (smsdvel/smavvel)*100
            if (actual_avevel < minvfil) :
            	reallystuck = 1
            else:
            	reallystuck = 0 
            ...
            self.mean_len_vel.append([filidcounter, actual_avelen, actual_stdlen, actual_avevel, actual_stdvel, smavvel, smsdvel, actual_maxvel, maxvsmooth, path_length, isstuck, reallystuck, untolval, smtolval])
            if actual_avelen < max_length and path_length >= min_path_length and untolval <= fil_tolerance and reallystuck ==0 :
            	self.final_len_vel.append([filidcounter, actual_avelen, actual_stdlen, actual_avevel, actual_stdvel, smavvel, smsdvel, actual_maxvel, maxvsmooth, path_length, isstuck, reallystuck, untolval, smtolval])

            	
            	
			  

# need to deal with the means here somehow     
            
        self.full_len_vel = np.array(self.full_len_vel)
        self.max_len_vel  = np.array(self.max_len_vel)
        self.mean_len_vel  = np.array(self.mean_len_vel)  # added NB
        self.final_len_vel  = np.array(self.final_len_vel)   # added NB
        if len(self.final_len_vel) == 0:   # added NB
        	print ("no filaments remain after filtering - relax the parameters?")   # added NB
        	return
         
        #If there is no path constructed-there won't be any velocity data
        ...
        # now do the filtering of this list - neil B - note his is only filtering the invidual 5 point averages 
        ...
        self.filt_len_vel     = self.full_len_vel[valid_filtered]
    ...
    def plot_2D_path_data(self, num_points, extra_fname=None):
      ...
      # np.bool no longer existsm, try dtype=bool. NB
        ...
        for i in range(len(filtered_paths)):
            ...
            for j in range(len(path.links)):
                ...
                py.figure(2000)
                py.arrow(mp_x2,mp_y2,mp_x1-mp_x2,mp_y1-mp_y2,color=path_colors[i],head_width=1,head_length=2,alpha=1.0)
            ...
            py.figure(2001)
            py.text(mp_mean[0],mp_mean[1],"%.f"%(mean_velocity),fontsize=6,color='k')
        ...
        ax.xaxis.set_visible(False)
        ax.yaxis.set_visible(False)
        py.savefig(self.directory+os.sep+'paths_2D.png',dpi=400,transparent=False)
...
# NB added this to get velocity info out - doesn't work well, compare to other means later    
        
        if not extra_fname == None:
            np.savetxt(extra_fname+'pathstats.txt',self.path_stats)        
        py.close('all')
    ...
    def write_path_data(self, extra_fname = None):
        #Write paths info
        f = open(self.directory+os.sep+'paths.txt','w')
    ...
    def plot_1D_path_data(self, min_path_length = 10, include_stuck = False, max_velocity = 1000, show_length = False, extra_fname=None):
        ...
        np.savetxt(self.directory+os.sep+'paths_mean_vel_projection.dat',mean_vel_project)
        np.savetxt(self.directory+os.sep+'paths_std_vel_projection.dat',std_vel_project)
        ...
        py.savefig(self.directory+os.sep+'paths_1D.png',dpi=200,transparent=True)
        ...
        #Save path stats
        np.savetxt(self.directory+os.sep+'paths_stats.dat',self.path_stats,header='first-frame stuck path-length mean-length mean-vel mean-std')
    ...
    def process_frame_links(self,num_frames_ave, percent_tolerance, min_path_length, max_length, fil_tolerance, minvfil):
        '''
        ...
        self.path_velocities(num_frames_ave,percent_tolerance, min_path_length, max_length, fil_tolerance, minvfil)
    ...
    def make_movie(self,extra_fname=None):
        ...
        avicommand = "ffmpeg -y -f image2 -r 6 -i skeletons_%03d.png -vcodec mpeg4 -q:v 12 -an -pix_fmt yuv420p filament_tracks.mp4"
        os.system(avicommand)
        
        #Copy the movie to output directory
        if not extra_fname == None:
            copy_command = "cp filament_tracks.mp4 "+extra_fname+"filament_tracks.mp4"
    ...
    def read_frame(self,num_frame,force_read = False):
        ...
        filament_file = self.directory+os.sep+'filXYs%03d.npy'%num_frame
    ...
    def write_length_velocity(self, header='',extra_fname=None):
        ...
        np.savetxt(self.directory+os.sep+header+'full_length_velocity.txt', self.full_len_vel)
        np.savetxt(self.directory+os.sep+header+'max_length_velocity.txt', self.max_len_vel)
        np.savetxt(self.directory+os.sep+header+'filtered_length_velocity.txt', self.filt_len_vel)
        np.savetxt(self.directory+os.sep+header+'mean_length_velocity.txt', self.mean_len_vel, fmt='%1.2f', delimiter='\t\t',header="FILID\t\tavLEN\t\tsdLEN\t\tavVuns\t\tsdVuns\t\tavVsmth\t\tsdVsmt\t\tmxVuns\t\tmxVsmt\t\tpathL\t\tSTUCK?\t\tVSTUCK\t\tSD/V%u\t\tSD/V%s")
        np.savetxt(self.directory+os.sep+header+'filament_length_velocity.txt', self.final_len_vel, fmt='%1.2f', delimiter='\t\t',header="FILID\t\tavLEN\t\tsdLEN\t\tavVuns\t\tsdVuns\t\tavVsmth\t\tsdVsmt\t\tmxVuns\t\tmxVsmt\t\tpathL\t\tSTUCK?\t\tVSTUCK\t\tSD/V%u\t\tSD/V%s")

        finalarray=np.delete(self.final_len_vel, np.where( np.bitwise_or( (self.final_len_vel[:,10]==1), (self.final_len_vel[:,11]==1) ) )[0], 0)
        np.savetxt(self.directory+os.sep+header+'FINALDOCUMENT.txt', finalarray, fmt='%1.2f', delimiter='\t\t',header="FILID\t\tavLEN\t\tsdLEN\t\tavVuns\t\tsdVuns\t\tavVsmth\t\tsdVsmt\t\tmxVuns\t\tmxVsmt\t\tpathL\t\tSTUCK?\t\tVSTUCK\t\tSD/V%u\t\tSD/V%s")
       
        # 3 lines above added Neil B         
        if not extra_fname == None:
            ...
            np.savetxt(extra_fname+'filtered_length_velocity.txt',self.filt_len_vel)
            np.savetxt(extra_fname+'mean_length_velocity.txt',self.mean_len_vel, fmt='%1.2f', delimiter='\t\t',header="FILID\t\tavLEN\t\tsdLEN\t\tavVuns\t\tsdVuns\t\tavVsmth\t\tsdVsmt\t\tmxVuns\t\tmxVsmt\t\tpathL\t\tSTUCK?\t\tVSTUCK\t\tSD/V%u\t\tSD/V%s")
            np.savetxt(extra_fname+'filament_length_velocity.txt',self.final_len_vel, fmt='%1.2f', delimiter='\t\t',header="FILID\t\tavLEN\t\tsdLEN\t\tavVuns\t\tsdVuns\t\tavVsmth\t\tsdVsmt\t\tmxVuns\t\tmxVsmt\t\tpathL\t\tSTUCK?\t\tVSTUCK\t\tSD/V%u\t\tSD/V%s")

            finalarray=np.delete(self.final_len_vel, np.where( np.bitwise_or( (self.final_len_vel[:,10]==1), (self.final_len_vel[:,11]==1) ) )[0], 0)
            np.savetxt(extra_fname+'FINALDOCUMENT.txt', finalarray, fmt='%1.2f', delimiter='\t\t',header="FILID\t\tavLEN\t\tsdLEN\t\tavVuns\t\tsdVuns\t\tavVsmth\t\tsdVsmt\t\tmxVuns\t\tmxVsmt\t\tpathL\t\tSTUCK?\t\tVSTUCK\t\tSD/V%u\t\tSD/V%s")
    ...
    def save_links(self):
        ...
        '''
        print("saving links")
        self.frame_links=np.asarray(self.frame_links, dtype=object)
        np.save(self.directory+os.sep+'links.npy' ,self.frame_links, allow_pickle=True)
        print("saved links")
    ...
    def plot_length_velocity(self,header='',extra_fname=None,max_vel=2400, max_length= 10000,nbins=30, min_points=2, min_path_length = 5, weighted=True, percent_tolerance=500, print_plot=True, minimal_plot=False, maxvel_color='b', plot_xlabels = True, plot_ylabels = True, square_plot = True, plot_length_f=False, fit_f = 'exp',dpi_plot=200):
        ...
        for filter_value in tolerance_list[::-1]:
            ...
            if len(valid_points) > 10:
                ...
                if len(non_stuck) > 0:
                    ...
                    # removed excess close bracket NB
                  
                    top_1_num = int(top_1_num)
                    top_5_num = int(top_5_num)
                    
                    # adding the ints above to make this list an integer length - Neil B                    
        ...
        print ('num all datapoints is  - %d points'%(num_points_all))   
        ...
        print ('num stuck datapoints is  - %d points'%(num_points_stuck))     
        ...
        print ('num moving datapoints is  - %d points'%(num_points_mobile))
        
                #Valid points - only moving, satisfying smooth movement tolerance
        valid_filtered      = np.nonzero((self.full_len_vel[:,1] > 0)*(self.full_len_vel[:,2] <= percent_tolerance/100.0*self.full_len_vel[:,1])*(self.full_len_vel[:,3] >= min_path_length))[0]
        num_points_filtered = len(valid_filtered)
        
        print ('num moving, smooth datapoints is  - %d points'%(num_points_filtered))
        ...
        if print_plot:
            if minimal_plot:
                ...
                axTolerance2.text(300,ylim[1]+0.1*tol_diff,r'$x10^3$',fontsize=15)
                ...
                axScatter.text(0,max_vel,r'$x10^3$',fontsize=15)
            ...
            py.savefig(self.directory+os.sep+header+'length_velocity.png',dpi=dpi_plot,transparent=False)
            if not extra_fname == None:
    ...
    def plot_correlation_profile(self,extra_fname = None):
        ...
        py.savefig(self.directory+os.sep+'correlation_length.png',dpi=200)
        if not extra_fname == None:
...
class Frame:
    ...
    def read_frame(self, frame_no):
        ...
        '''
        fname         = self.directory+os.sep+self.header+'%03d'%frame_no+'_'+self.tail+'_000.tif' 
 ...
 #       self.img      = cv2.imread(fname,cv2.IMREAD_GRAYSCALE)
    ...
    def save_filament_img(self):
        ...
        ax.xaxis.set_visible(False)
        ax.yaxis.set_visible(False)
        py.savefig(self.directory+os.sep+'filaments_%03d.png'%(self.frame_no),dpi=200)
        py.close()
    ...
    def save_filXYs(self):
        ...
        filament_file = self.directory+os.sep+'filXYs%03d'%self.frame_no

        self.filXYs=np.asarray(self.filXYs, dtype=object) 

     
        np.save(filament_file,self.filXYs, allow_pickle=True)
   
    def read_filXYs(self):
        '''
        Load the filXYs for filament reconstruction
        '''
        filament_file = self.directory+os.sep+'filXYs%03d.npy'%self.frame_no
...
# line below is abn attempt to fix the problem after new numpy version
        self.xy_norm=tuple(map(tuple, self.xy_norm))
...
class Filament:
    ...
    def calc_fil_length(self):
        self.num_contour_pixels = len(self.contour)
        dist = self.contour[1:,:] - self.contour[:-1,:]
        
        
        self.fil_length = np.sum(np.sqrt(np.sum(dist**2,axis=1)))
    # NB - this doesn't seem like a true measurement of length 
    ...
    def calc_fil_stats(self):
        ...
        self.midpoint     = self.contour[int(len(self.contour)/2-1)]
